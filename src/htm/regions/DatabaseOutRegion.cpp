...
void DatabaseOutRegion::initialize() {
  ...
  NTA_ASSERT(in) << "DatabaseOutRegion::init - 'dataIn' input not configured\n";

  if (!in->hasIncomingLinks() || in->getData().getCount() == 0) {
    NTA_THROW << "DatabaseOutRegion::init - no input Data found\n";
  }
  dataIn_ = in->getData();
}
...
void DatabaseOutRegion::openFile(const std::string &filename) {
  ...
  if (result!=0 || dbHandle==nullptr)
  {
    ...
    NTA_THROW
        << "DatabaseOutRegion::openFile -- unable to create or open database file: "
        << filename.c_str()
		<< " Error code:"
		<< result;
  }
...
Spec *DatabaseOutRegion::createSpec() {
  ...
  auto ns = new Spec;
  ns->description =
      "DatabaseOutRegion is a node that simply writes its "
...
size_t
DatabaseOutRegion::getNodeOutputElementCount(const std::string &outputName) const {
  NTA_THROW
